...
def world(user_conf=None, project_conf=None, session_conf=None, recorded=None, git=True):
    ...
    for path, text in ((os.path.join(home, CONFIG_NAME), user_conf),
                       (os.path.join(repo, PROJECT_DIR, CONFIG_NAME), project_conf),
...
def run(home, cwd, *argv, session=SESSION):
...
def project_conf(repo):
    return conf(repo, PROJECT_DIR, CONFIG_NAME)
...
check("with nothing set, the shipped default is what is in force",
      code == 0 and f"{DEFAULT:,} tokens" in out, f"{code} {out} {err}")
check("and show says so for this session and for a new one alike",
      out.count(f"{DEFAULT:,} tokens") == 2, out)
...
check("and show still reports the default for a new session here",
      f"a new session here  {DEFAULT:,} tokens" in out, out)
...
check("and no second file appears in the subdirectory",
      conf(repo, "sub", PROJECT_DIR, CONFIG_NAME) is None,
      str(conf(repo, "sub", PROJECT_DIR, CONFIG_NAME)))
...
deeper = os.path.join(repo, "sub", PROJECT_DIR, CONFIG_NAME)
...
check("the project file already in force here is the one rewritten",
      code == 0 and conf(repo, "sub", PROJECT_DIR, CONFIG_NAME) == "ceiling = 350000\n",
      str(conf(repo, "sub", PROJECT_DIR, CONFIG_NAME)))
...
home, repo = world(recorded=f"ceiling = {DEFAULT}\n")
code, out, err = run(home, repo, "set", "project", "+100_000")
...
home, repo = world()
other = scratch_dir()
os.makedirs(os.path.join(other, PROJECT_DIR))
...
check("and leaves another project's layer alone",
      conf(other, PROJECT_DIR, CONFIG_NAME) is None, str(conf(other, PROJECT_DIR, CONFIG_NAME)))
...
check("and returns the session to the ceiling beneath it",
      f"this session        {DEFAULT:,} tokens" in out, out)
...
check("and this session returns to the ceiling it started under, not to what the files say now",
      f"this session        {DEFAULT:,} tokens" in out and conf(
...
home, repo = world()
code, out, err = run(home, repo, "clear", "project")
check("clearing a project that never set a ceiling reports both layers absent",
      code == 0 and out.count("absent") == 2, f"{code} {out}")
