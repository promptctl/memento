...
1. Propagate its current choice to this repo first (its per-repo install step), then `gh run rerun "$RUN_ID"` — the head SHA is unchanged, so only a rerun re-fires the reviewer — and go back to `provider.wait`. Another session may have rotated already, and this repo's secret lags behind that choice until the installer runs here; the secret is write-only, so there is no way to read which account the failed run used. Propagating and rerunning answers that question without knowing it.
2. If that rerun is limited too, the account the setup skill names is spent: rotate per that section onto an account that still has capacity, propagate to this repo, rerun, and wait again.

Repeat 2 until a rerun reviews the head. Stop and surface only once every account the setup skill knows of has been tried and limited, and say so with the reset time the log names.
...
- **A usage-limit failure (`action` provider) is a credential to swap, not a wait to report.** The reviewer's account is one of a pool the setup skill keeps. When the failed log carries the action's `##[error]rate-limited:` annotation, propagate its current choice to this repo and rerun the run; if that is limited too, rotate per its *Rotating the reviewer account*, propagate, rerun, and continue. Waiting out a multi-day reset, or reviewing locally instead, is the loop stalling with the fix in reach.
