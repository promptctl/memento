...
**`action` provider — a usage-limited run.** The run id is the last path segment of `url`. Ask GitHub for the failed job's check-run annotations and look for the action's own failure annotation:
...
1. Propagate its current choice to this repo first (its per-repo install step), then `gh run rerun "$RUN_ID"` — the head SHA is unchanged, so only a rerun re-fires the reviewer — and go back to `provider.wait`. Another session may have rotated already, and this repo's secret lags behind that choice until the installer runs here; the secret is write-only, so there is no way to read which account the failed run used. Propagating and rerunning answers that question without knowing it. The installer converges the workflow file as well as the secret, and prints which it did: if it wrote the workflow file rather than reporting it up to date, the template has moved since this repo last installed. The secret is set either way, so the rerun proceeds; the rewritten file is not this PR's change — drop it from the working tree and note in the recap that this repo's workflow lags the template.
2. If that rerun is limited too, the account the setup skill names is spent: rotate per that section onto an account that still has capacity, propagate to this repo, rerun, and wait again. Keep a list of the accounts you have landed on this loop, starting with the one the setup skill named in step 1.

Repeat 2 until a rerun reviews the head. The pool is finite and the setup skill lists it; once every listed account is on your list and has failed a rerun, stop and surface. Name each tried account's reset time from its run's log — the earliest is when the loop can resume, and the last log's figure alone is not the answer.
...
**A body finding (`thread_id` null)** has no thread — nowhere to post the plan comment, nothing for `provider.resolve` to take. It lives in its blocking review's body, so `fetch` keeps returning it for as long as that review stays CHANGES_REQUESTED, whatever the head SHA — and dismissing that review, with the finding's disposition recorded in the dismiss message (step 8), is the one thing that clears it. Classify it the same way: a change-needed body finding is fixed and pushed like any other (steps 4/6), but that fix+push earns its keep by making the *next* review come back clean so the finding does not recur — it does not clear the current finding, which only the step-8 dismissal clears. A no-change body finding (invalid, already-fixed) needs no push — recording its disposition in the dismiss message is the whole action. Either way there is nothing to `resolve` in step 7; either way the step-8 dismissal is not optional, even once the new head re-reviews clean.
...
[LAW:single-enforcer] if a `/goal` is active in this session, carry it: pass `--goal '<the exact condition>'` as the leading argument, ahead of the message, exactly as message-in-a-bottle's contract requires. An un-carried goal silently dies at the close-out — a multi-PR goal (e.g. "every open PR on this branch is merged") would stop at this first PR. Omit `--goal` entirely when no goal is set.
