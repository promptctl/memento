...
[LAW:no-silent-failure] if `conclusion` is anything other than `success`, the reviewer itself errored — its findings are absent, not empty. Do not treat a failed run as a clean review. Stop and surface the run `url` — with one exception, for the `action` provider only, where the loop fixes the cause itself. Every other provider stops here on any non-success conclusion; the mechanics below are GitHub Actions mechanics and mean nothing to them.

**`action` provider — a usage-limited run.** The run id is the last path segment of `url`. Read the failed log for the action's own error annotation:

```bash
gh run view "$RUN_ID" --log-failed | grep -F '##[error]rate-limited:'
```

Match on that annotation and nothing looser. The action emits it from its own process after exhausting its retries, whereas the model's message ("You've hit your limit · resets …") also appears wherever the log echoes the diff — a PR that quotes that string anywhere would send a healthy run down this path. It happened to this skill's own PR.

A match means the Claude account behind this repo's `CLAUDE_CODE_OAUTH_TOKEN` secret is out of usage quota. The run never read the diff, and it fails identically on every rerun until that account's quota resets, typically days out. **A usage limit is a credential problem, and the fix is to swap credentials, not to wait.** The credential comes from a pool of accounts kept by the `agent-code-review-setup` skill; its *Rotating the reviewer account* section (`~/.claude/skills/agent-code-review-setup/SKILL.md`) owns the mechanics — which accounts exist, how the reviewer's account is chosen, how a repo is moved onto it. Follow it in this order:

1. Read which account it currently names as the reviewer *before* changing anything. Another session may have rotated already — this repo's secret lags behind that choice until the installer is run here. If the named account is not the one that just failed, skip the rotation: propagate to this repo (its per-repo install step) and go to 3.
2. Otherwise rotate per that section, onto an account that still has capacity, and propagate to this repo.
3. `gh run rerun "$RUN_ID"` — the head SHA is unchanged, so only a rerun re-fires the reviewer — and go back to `provider.wait`.

If the rerun is limited too, the account you landed on is spent as well; repeat from 1. Stop and surface only once every account the setup skill knows of has been tried and limited, and say so with the reset time the log names.

You will be reading "You've hit your limit · resets <date>", the date will be two days out, and the thought will be *"nothing can move until then — report the wait and stop."* Its cousin: *"I'll review it locally in the meantime."* Neither. Waiting parks every open PR for days with the fix ten minutes away, and a local review is not the reviewer — `provider.fetch` is the only stream of findings this loop trusts (`[LAW:one-source-of-truth]`). The limit belongs to one account; the pool has more.
...
```bash
${CLAUDE_PLUGIN_ROOT}/skills/message-in-a-bottle/bin/finalize-session --reset clear "$(cat <<'EOF'
...
The reset is `clear`, not `compact`: the PR is merged, the ticket is closed and the recap is filed — a finished unit of work handing off to an unrelated next candidate, with nothing left to carry forward that the message above doesn't already carry.

[LAW:single-enforcer] if a `/goal` is active in this session, carry it: pass `--goal '<the exact condition>'` as the leading argument, ahead of `--reset clear` — both flags precede the message, exactly as message-in-a-bottle's contract requires. An un-carried goal silently dies at the close-out — a multi-PR goal (e.g. "every open PR on this branch is merged") would stop at this first PR. Omit `--goal` entirely when no goal is set.
...
- **A usage-limit failure (`action` provider) is a credential to swap, not a wait to report.** The reviewer's account is one of a pool the setup skill keeps. When the failed log carries the action's `##[error]rate-limited:` annotation, rotate per its *Rotating the reviewer account* — reading which account it names first, since a concurrent session may have rotated already — rerun the run, and continue. Waiting out a multi-day reset, or reviewing locally instead, is the loop stalling with the fix in reach.
