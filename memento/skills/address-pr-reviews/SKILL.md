...
1. Propagate its current choice to this repo first (its per-repo install step), then `gh run rerun "$RUN_ID"` — the head SHA is unchanged, so only a rerun re-fires the reviewer — and go back to `provider.wait`. Another session may have rotated already, and this repo's secret lags behind that choice until the installer runs here; the secret is write-only, so there is no way to read which account the failed run used. Propagating and rerunning answers that question without knowing it. The installer converges the workflow file as well as the secret: if it rewrote `.github/workflows/code-review.yml`, the template has moved since this repo last installed. The secret is set either way, so the rerun proceeds; the rewritten file is not this PR's change — drop it from the working tree and note in the recap that this repo's workflow lags the template.
2. If that rerun is limited too, the account the setup skill names is spent: rotate per that section onto an account that still has capacity, propagate to this repo, rerun, and wait again. Keep a list of the accounts you have landed on this loop.
