...
On success the launcher exits 0 having printed the outcome your flags chose. Without `--reset`: `handoff recorded → <path>` and `no reset: this session keeps its context, so carry on with the work.` With `--reset`: `handoff scheduled → <target> (/<mode>) in Ns (log: <tempfile>)` — the log captures worker progress and any transport errors.
